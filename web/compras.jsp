...
<html lang="es">
    ...
    <body style="font-family:'Nunito Sans', sans-serif" onLoad="">
        <%
        ...
        <section class="contenido-carrito" style="padding: 0 4rem;">
            <div class="">
                <h2 style="text-align: center; margin-top: 2rem;"><b>Compras</b></h2>
                <br>
                <span><b>Nota: </b>Para el seguimiento de los pedidos se manejan los siguientes estados:</span>
                <br><br>
                <ul>
                    <li><b>Recibido -> </b> El pedido ha sido registrado en el sistema, espera respuesta por la administración.</li>
                    <li><b>Pendiente pago -> </b> El pedido fue verificado y se espera el pago del mismo, se tiene un lapso de <b>1 día</b> para realizarlo.</li>
                    <li><b>Pago realizado -> </b> Se cargo al sistema la imagen con el comprobante de pago.</li>
                    <li><b>En proceso -> </b> Se comprobó el pago realizado, y el pedido está siendo preparado para su envío.</li>
                    <li><b>Finalizado exitosamente -> </b> El pedido fue entregado al trasporte de paquetes.</li>
                </ul>

                <div class="accordion" id="accordionExample">
                    <c:forEach var="pedidos" items="${listaPedido}">
                        <div class="">
                            <div class="card-header" id="${pedidos.getId_pedido()}" style="background: var(--color-fondo);">
                                <h2 class="">
                                    <a style="color: var(--color-fondo-oscuro); font-size: 1.25rem; font-weight: bolder; color: var(--color-secundario)" class="btn  btn-block text-left titulo-acordeon" type="button" data-toggle="collapse" data-target="#h${pedidos.getId_pedido()}" aria-expanded="true" aria-controls="h${pedidos.getId_pedido()}">
                                        <b>Pedido #${pedidos.getId_pedido()}</b>
                                    </a>
                                </h2>
                            </div>
                            <div id="h${pedidos.getId_pedido()}" class="collapse" aria-labelledby="${pedidos.getId_pedido()}" data-parent="#accordionExample">
                                <div class="card-body">
                                    <h4 style="text-align: center"><b>Descripción de Compra</b></h4>
                                    <section class="contenedor-tabla">
                                        <table class="table" style="text-align: center">
                                            <thead  style="background-color: var(--color-fondo-oscuro); color: white">
                                                <tr>                                                
                                                    <th scope="col">Sutotal</th>
                                            ...
                                            <tbody id="tbl_cliente" onLoad="asignarValor()">
                                                <tr>
                                                    <td style="display: none" >${cliente.getId_cliente()}</td>
                                                    <td>${pedidos.getSub_total()}</td>
                                                    <td>${pedidos.getTotal()}</td>
                                                    <td>${pedidos.getFecha()}</td>
                                                    <td>${pedidos.getPago()}</td>
                                                    <td>${pedidos.getGuia_transporte()}</td>

                                                    <td class="tbl-cliente" style="background-color: var(--color-fondo); color: var(--color-fondo-oscuro);">
                                                        <b>${pedidos.getSeguimiento()}</b>
                                                        <c:if test="${pedidos.getId_seguimiento()>1 && pedidos.getId_seguimiento()<3 && pedidos.getId_tipo_pago()<2}">
                                                            <a data-toggle="modal" data-target="#modal${pedidos.getId_pedido()}" ><i class="fa-solid fa-upload"></a></i> 
                                                        </c:if>
                                                    </td>
                                                </tr>                         
                                            </tbody>
                                    ...
                                    <div class="col-sm-12">
                                        <table class="table" style="text-align: center">
                                            ...
                                            <tbody class="carrito-tabla">
                                                <c:forEach var="detalle" items="${pedidos.getDetallecompras()}">                            
                                                    <tr>                                                
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="modal${pedidos.getId_pedido()}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content" style="background-color: var(--color-fondo);">
                                    <div class="modal-header modal-cabecera">
                                        <h5  id="staticBackdropLabel"><b>Adjuntar imagen </b><i style="font-size: 1.5rem; color: var(--color-secundario)" class="fa-solid fa-image"></i></h5>                          

                                    </div>
                                    <div class="modal-body">
                                        <form action="SrvProducto" method="post" enctype="multipart/form-data">
                                            <label><b>Elegir imagen</b></label>                        
                                            ...
                                            <input type="hidden" id="txt_idPedido" name="txt_idPedido" value="${pedidos.getId_pedido()}">                                
                                            <br><hr>
                                            <div class="pedido-op">
                                                <button class="button-pedido" type="submit" id="accion" name="accion" value="registrarPagoImg">Aceptar</button>                                    
            ...
            <div class="contenido-carrito-btn">
